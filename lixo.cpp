	...
	while(!Q -> empty()) {
		...
		if (u == s || u -> daddy != NULL) {
			...
			for (i = 0; i < size; i++) {
				...
				if(node != NULL && node -> color == 0 &&  u->daddy != node && u -> weight[i] != u -> currFlow[i] && u->weight[i] != 0) {
					//printf("a minha diferença do u é: %d e os ids sao: %d %d\n", u->weight[i] - u ->currFlow[i], u->id, node->id);
					//printf("u: %d Node: %d weight: %d currFlow: %d\n", u -> id, node ,-> id, u -> weight[i], u -> currFlow[i]);
					node -> daddy = u;
					...
					if(node != t)
						...
						//printf("FLOW: %d\n", flow);
 						while (node != s) {
							//printf("Estou no %d, o daddy e %d com o parent a %d\n", node -> id, node -> daddy -> id, node -> parent);
							node -> currFlow[node -> parent] += flow;
							//if (node -> currFlow[node -> parent] == node -> weight[node -> parent])
							//	printf("ENCHIIIIIIII %d %d\n", node -> id, node -> daddy -> id);
							node -> daddy -> currFlow[getChildId(node -> daddy, node)] += flow;
							temp = node -> daddy;
							node = temp;
							//printf("sou o no:%d\n", node ->id);
							contador++;
							//printf("parabens, cheguei aonde devia\n");
						}
						//printf("-----------------\n");
						if(contador > 2)
							totalFlow += flow;
